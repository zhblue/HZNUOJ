...
### 代码相似度检测开关 OJ_SIM_ENABLE、OJ_SIM_SWITCH 和 $OJ_SIM

OJ_SIM_ENABLE负责标记判题机是否开启代码相似度检测

OJ_SIM_SWITCH负责切换相似度检测程序

$OJ_SIM负责标记web部分的状态页上是否显示代码相似度检测的结果。

通过配置judge.conf中的OJ_SIM_ENABLE = 1 和 static.php中的$OJ_SIM = true 开启代码相似度检测，也就是抄袭检查功能，但只针对AC的代码，不检测没通过的代码。

判题机通过调用第三方应用程序对**AC的提交代码**进行语法分析判读文本相似度，通过检验的代码将由判题机复制进题目数据的 ac 目录成为新的参考样本（基于此，长期开启本功能会占用大量硬盘存储空间）。

通过配置judge.conf中的OJ_SIM_SWITCH选择不同的比对程序。OJ_SIM_SWITCH = 1：使用**SIM**进行相似度检测，速度较快；OJ_SIM_SWITCH = 2：使用Python3的**CopyDetect**从语义、文本两方面进行相似度检测，速度相对较慢，当标定为200%相似度时，表明两个代码一模一样。
