...
function canSeeSource($sid) {
    ...
    global $mysqli;
    /* 获取solution信息 start */
    $sql="SELECT `solution`.*, `contest`.`defunct`, `contest`.`practice`, `contest`.`open_source` FROM `solution` 
    ...
    if (isset($_SESSION['user_id'])&&$row && $row->user_id==$_SESSION['user_id']){
        ...
        $ok = is_numeric($cid) ? $pri_in : $pri_out;
        if(!$ok) {
...
function get_contests($type_list,$filter=" AND 1 "){ //返回一个二维数组给选择框等提供比赛场次数据
    ...
    foreach ($type_list as $key => $value) {
        switch($key){
            case "Special":
                $sql="SELECT `contest_id`,`title`,`defunct` FROM `contest` WHERE NOT `practice` AND `user_limit`='Y' $filter ORDER BY contest_id DESC";//类型优先级2
                $result=$mysqli->query($sql);
            ...
            break;
            case "Private":
                $sql="SELECT `contest_id`,`title`,`defunct` FROM `contest` WHERE NOT `practice` AND `user_limit`='N' AND `private` $filter ORDER BY contest_id DESC";//类型优先级3
                $result=$mysqli->query($sql);
            ...
            break;
            case "Public":
                $sql="SELECT `contest_id`,`title`,`defunct` FROM `contest` WHERE NOT `practice` AND `user_limit`='N' AND NOT `private` $filter ORDER BY contest_id DESC";//类型优先级3
                $result=$mysqli->query($sql);
            ...
            break;
            case "Practice":
                $sql="SELECT `contest_id`,`title`,`defunct` FROM `contest` WHERE `practice` $filter ORDER BY contest_id DESC";//类型优先级1
                $result=$mysqli->query($sql);
            ...
            break;
            case "All":
                $sql="SELECT `contest_id`,`title`,`defunct` FROM `contest` WHERE `defunct`='N' ORDER BY contest_id DESC";
                $result=$mysqli->query($sql);
                $view_contest['All']['type'] = $MSG_Practice;
                $view_contest['All']['data'] = $result->fetch_all(MYSQLI_ASSOC);
                $view_contest['All']['disabled'] = $value ? "" : "disabled";
...
function getContestEndtime($uid, $contestid){
    if(!is_numeric($contestid) || $uid=="") return 0;
    global $mysqli;
